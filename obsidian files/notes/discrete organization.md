...
> [!info]+ Definition of **graphe edges**
> A **graphe edge** is an edge on the KG, which connects two GPs together. Each edge represents a rule, which determines the subset of KIs of the child GP.
...
> A **core concept** is piece of information which is self describing (it is described by its nature alone). (These exist, because, otherwise, there would be no way to describe anything, as everything would just point to something else, which points to something else.)
...
2. A number $n$ of child GPs are created form $U$. Each child contains a subset of the KIs contained in $U$ based on the rule chosen in the GEs.
