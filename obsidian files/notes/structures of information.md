...
Additionally, it is important to note that structures of information are intended to describe a concept (the *structure object*), which means that ==an instance of a given structure is only a small part of the potentially infinite set of things that can be described using the structure.== For example, a guitar song described by sheet music is only one of an infinite number of songs which can be described using sheet music. This might seem obvious, but stating it clearly will make things make more sense when we start describing more theoretical structure objects.
