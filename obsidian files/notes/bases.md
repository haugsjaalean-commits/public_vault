...
3. Use the command from the command palette
...
In the top left of the base, we have the option to edit the different views of the base. As I will explain in sorting, each view has its own unique filters. This means that each view can be a more focused version of the greater base. 
...
If we click on the options for one of the views, we have the option to change the way it's being displayed (the "layout") and other options which are unique to each layout, like the "image property" for the "cards" layout.
...
This is very intuitive. We can choose a property to group by, and we can have any number of properties used to sort by.  
...
2. **This view**: These filters are used to further narrow down the specific view of a base. In the screen shot above, I use this section to search for only artists whose **medium** property (of type **list**) contains **pen and ink**.
...
I put **Properties** and **Search** together, because they are complementary to each other. 
...
2. Go from *general* to *specific* with views: It's always important to start off by trying to make the base itself as general as possible. This way, you have more freedom in creating the individual views in the base, and you are less likely to create multiple bases which do the same or similar things. 
